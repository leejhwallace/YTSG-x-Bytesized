document.addEventListener('DOMContentLoaded', async () => {
            ...
            try {
                    ...
                    if (currentUserDisplayName) {
                            items = rawItems.filter(item => {
                                    ...
                                    const assigneesArray = String(item.assignee).split(',').map(s => s.trim());
                                    return assigneesArray.includes(currentUserDisplayName);
                            });
                    } else {
                            items = rawItems;
                    }
        ...
        saveButton.addEventListener('click', async () => {
                ...
                if (activeType === 'project') {
                        ...
                        if (error) {
                                ...
                                alert("Failed to save event. Check console for exact error.");
                                return;
                        }
                }
    ...
    const renderCalendarItems = (year, month) => {
            ...
            // FIX: Separate projects from events first so events don't get processed as "Master Projects"
            const projectItems = visibleItems.filter(i => i.type === 'project');
            const eventItems = visibleItems.filter(i => i.type === 'event');
            
            // -- HIERARCHICAL RENDERING FOR PROJECTS ONLY --
            ...
            // Re-merge perfectly sorted projects with standard events
